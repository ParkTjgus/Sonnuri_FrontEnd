...
const VideoRecordBox =
    ({ videoRef, isLoading, isError, isActive, setReady }: VideoRecordBoxProps ) => {
        ...
        if (!isActive) {
            return <DefaultVideoBox content={
                <BsCameraVideoFill color="gray" size={30} />
            } />
        }

        if (isLoading) {
            return <DefaultVideoBox
                content={<h2 className="flex gap-[20px] items-center">
                    <FiLoader size={30} />로딩 중...</h2>} />
        }
        if (isError) {
